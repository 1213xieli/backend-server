...
public interface WxAppTokenMapper {
    ...
    // fid=Va4Arehrhe7474ugR
    @Update("insert into T_APP_WeiXinToken (fid,FAPPID,FTokenId) values (#{fid},#{appId},#{token})")
    void addWxAccessToken(@Param("fid") String fid, @Param("appId") String appId, @Param("token") String token, @Param("appSecret") String appSecret);
}
